    ...
    return (
        <div className="w-full min-h-screen pt-24 md:pt-32 px-5 lg:px-32 flex flex-col">
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="pb-14">
                ...
                <motion.p variants={item} className="text-black/60 font-mono text-lg">
            ...
            <motion.div variants={container} initial="hidden" animate="show">
                {projects.map((project, idx) => (
                    <div key={idx} className="grid grid-cols-[2fr_4fr_1fr] gap-2 pb-[10vh]">
                        <motion.div variants={item} className="pt-5">
                            <div className="font-bold text-2xl leading-loose font-sans">
                                {project.name}
                            </div>
                            <p className="font-mono text-lg text-black/60">{project.year}</p>
                        </motion.div>
                        
                        <motion.div variants={item} className="w-full h-full">
                            <div className="h-full w-full relative pb-10 pr-20">
                                {/* Foreground card */}
                                <div className="shadow-md rounded-3xl p-5 backdrop-blur-md w-full h-full grid grid-rows-[2fr_1fr] relative">
                                    <a 
                                        className="w-16 h-16 absolute right-2 top-2 hover:scale-110 transition-transform" 
                                        target="_blank" 
                                    ...
                                    <div className="text-xl text-black/60 font-mono">
                                        {project.description}
                                    </div>
                                </div>
                            </div>
